...
import { fleetTheme as T } from "../../constants/theme";
import {
  FormField,
  StyledInput,
  PrimaryButton,
...
export default function AssignCarScreen({ navigation, route }) {
  ...
  async function loadAcceptedDrivers() {
    setDriverSearching(true);
    try {
      const results = await searchDrivers(""); // empty query = return all accepted
      ...
      <View style={styles.root}>
        ...
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            value={driverQuery}
            onChangeText={loadAcceptedDrivers}
            autoFocus
          />
          {driverSearching && (
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                // Auto-fill job title
                if (item.accepted_jobs.length === 1) {
                  // Only one job — auto-fill directly
                  setJobTitle(item.accepted_jobs[0].title);
                  setJobOptions([]);
                  setShowJobDropdown(false);
                } else if (item.accepted_jobs.length > 1) {
                  // Multiple jobs — show dropdown
                  setJobOptions(item.accepted_jobs);
                  setJobTitle("");

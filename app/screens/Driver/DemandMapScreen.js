...
const DemandMapScreen = ({ navigation }) => {
  const { insights, loading, error, fetchInsights } = useDemandStore();
  ...
  useEffect(() => {
    if (error) {
      Toast.show({ type: 'error', text1: 'Connection issue', text2: 'Could not load demand data. Pull to refresh.' });
    }
  }, [error]);
